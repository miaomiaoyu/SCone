...
try
    ...
    sizeOfSquare=100;
    transLayer=2;
    numToChangeToMakeCircleClearer=1000;
    sizeOfBlob=255;
    [x,y]=meshgrid(-sizeOfSquare:sizeOfSquare, -sizeOfSquare:sizeOfSquare);
    maskblob=uint8(ones(2*sizeOfSquare+1, 2*sizeOfSquare+1, transLayer) * black);
    size(maskblob);
    ...
    maskblob(:,:,transLayer)=uint8(round(-sizeOfBlob + exp(-((x/xsd).^2)-((y/ysd).^2))* numToChangeToMakeCircleClearer));
    ...
    while (1)
        ...
        if (mx~=mxold || my~=myold)
            myrect=[mx-sizeOfSquare my-sizeOfSquare mx+sizeOfSquare+1 my+sizeOfSquare+1]; % center dRect on current mouseposition
