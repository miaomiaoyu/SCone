...
UsingVP = true;
useHardwareStereo = true; %Setting to true works much better on the Viewpixx (+mac).
...
if UsingVP
    ...
    elseif Datapixx('IsPropixx') % If it's the PROpixx projector
        ...
        load('PROpixx_Processed_cal_data_21_4_2016.mat')
...
try %Start a try/catch statement, in case something goes awry with the PTB functions
    ...
    if Load2Deg % for 2 deg fundamentals
        ...
        %load('\\PSHome\Home\rm1380\My Documents\Colour\StockmanSharpe_10deg_cone_fundamentals_1nm.mat')
    ...
    results.DotLifetimeMS = 1000/(RefreshRate/2) * fperiod/2; %=> the dot lifetime for a single colour.
        ...
        while ~IsoAccept
            ...
            if keyIsDown %If they've pressed a button on keyboard,
                ...
                elseif any(keyCode(Accept))
                    ...
                    results.theta_vals(trial) = theta;
    ...
    results.MeanThetas(1) = mean(results.theta_vals(1:2:end)); % mean theta: left eye
    results.MeanThetas(2) = mean(results.theta_vals(2:2:end)); % mean theta: right eye
    ...
    results.FlickerFrequency = 1000/(results.DotLifetimeMS * 2);
    
    % Save the results:
    save(fName, 'results')
    
    %Print out the results:
    fprintf('\nThetas  = %.3f', results.theta_vals);
    fprintf('\n \nMean theta, left eye = %.3f \n \nMean theta, right eye = %.3f\n', results.MeanThetas(1), results.MeanThetas(2));
    fprintf('\nMissed frames = %.3f \n', missedFrames)
